...
- **Real Postgres for Verify:** there's no docker daemon, but Postgres 16 runs locally via `pgserver` in a scratch venv (binaries in `/usr/lib/postgresql/16`). Set `OPENAD_TEST_PG_URL` so `api/tests/test_migrations.py` (and other PG-gated tests) run against it; otherwise they skip. CI does the same with a `postgres:16` service and a "pytest with Postgres" step (PR #15). Parallel worktrees use separate databases on the same pgserver socket: `openad_test` (39) and `openad_test_q` (41).
...
- **Periods endpoint (step 41, accepted 2026-09-25):** on main (`5f27fb8`), `GET /v1/slots/{id}/periods` has no range cap and reads leases one index at a time. 41 caps it at 60 periods (422 `invalid_window`) with one lease query (T19).
