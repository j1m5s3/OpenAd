...
- **Real Postgres for Verify:** there's no docker daemon, but Postgres 16 runs locally via `pgserver` in a scratch venv (binaries in `/usr/lib/postgresql/16`). Set `OPENAD_TEST_PG_URL` so `api/tests/test_migrations.py` (and other PG-gated tests) run against it; otherwise they skip. CI does the same with a `postgres:16` service and a "pytest with Postgres" step (PR #15). Parallel worktrees use separate databases on one server. For the slice-review fixes (D14) that server's socket is `/tmp/pgdata_jit16`: `openad_test` (45) and `openad_test_42`, `_43` and `_44`, via `OPENAD_TEST_PG_URL='postgresql+asyncpg://postgres@/<db>?host=/tmp/pgdata_jit16'`.
...
- **JIT files location:** since slice B merged, the uncommitted JIT edits ride with the branch checked out in the primary tree (`/home/claude/OpenAd`). Step 37 committed them as `2ce0f5b`. Later edits ride with `chore/launch-final` (draft PR #16: steps 36, 36b, 46 and 47). The planner's CLOSE archive also lands there, after 47 and the orchestrator's targeted re-review (D14). Coders never stage `.cursor/`.
...
- **Outbound fetch hazard (found 2026-09-25; fixed by step 39, PR #17 `3605473`):** httpx `timeout=` is per operation (each read), not a total deadline. Wrap every outbound fetch in `asyncio.timeout`, read raw bytes (`Accept-Encoding: identity`, `aiter_raw`) under a byte cap, and never hold a DB session across network I/O (read, commit, fetch, write). As shipped: `OPENAD_MEDIA_FETCH_DEADLINE_SECONDS` 30, `OPENAD_VERIFY_PASS_BUDGET_SECONDS` 20, `_check_meta` ≤ 3 hops under 38's rules with a 10 s deadline and a 256 KiB or `</head>` cap, and a per-slot 30 s cooldown as one atomic `UPDATE` (429). Residuals (T18): no per-address limit on creative verify and no per-pass concurrency (7.14); DNS rebinding stays with T17 (7.12).
- **Calendar rule for UI (step 40, PR #19 `f50076d`):** periods are open-ended (`start_k = first_period_start + k·period_seconds`). A period is buyable, time-wise, when `max(0, start_k − lead) ≤ now < end_k` and (`sale_end == 0` or `end_k ≤ sale_end`), with LEASE terms, `lead > 0`, not paused and not leased. See the PROTOCOL §4.1–4.2 `buy` checks and `api/src/openad/services/periods.py`. Never derive a UI state or a period list from the first period alone. Web (`web/src/lib/auction.ts`): `auctionStatus` checks no terms → paused → CPC → no calendar first (paused wins over CPC); the slot page lists `periodsWindowSize(lead, period) + 1` periods (`min(59, max(14, ceil(lead / period)))`, so 15–60) from the calendar-only `currentPeriodIndex`, which mirrors `models/slot.py`. Tests, e2e and the screenshot script find period rows by their index cell, never by position.
- **Periods endpoint (step 41, PR #18 `289bb72`):** `GET /v1/slots/{id}/periods` rejects `to − from + 1 > 60` with 422 `invalid_window`, bounds `from` and `to` each at `UINT256_MAX`, and reads leases in one query (an index scan on `pk_leases`) (T19). API defaults are `from=0`, `to=7`. Callers stay under the cap: web 15–60, sim 8, sim planner 5.
...
- **Slice-review fixes (D14; REPLAN 2026-09-25 10:05 UTC):** the Opus slice review of main `f50076d` plus #16 returned FIX (six L1s, eight L2s). The fixes are coded in parallel off `f50076d`, each in its own worktree: 42 `fix/deploy-images`, 43 `fix/cloud-run-wiring`, 44 `fix/settler-key` and 45 `fix/cpc-click-integrity`, in `/home/claude/OpenAd-42` … `-45`. 46 (docs truth) runs on #16 in the primary tree. Then come 47 (merge main into #16, record 6.11–6.14), a targeted re-review, and CLOSE. D14 lists hunk ownership for every shared file. Pre-assigned: T20 (44); T13 amended (45); ROADMAP 6.11–6.14 (42–45, written by 47); 7.19 (46). 42–45 never edit ROADMAP or README. Only 46 runs the default-port Playwright suites (4173 and 4174); 42's boot check uses ports 5181–5189.
- **Image and Cloud Run rules (D15; being built in 42 and 43):** the api image carries `contracts/deployments` at `/app/contracts/deployments` and sets `OPENAD_DEPLOYMENTS_DIR`; `REPO_ROOT` is `/` in the image, and the indexer and settler load the artifact before their liveness listener starts. An empty `VITE_*` build input means unset: RainbowKit throws "No projectId found" as soon as a WalletConnect-based wallet is created, so with no id the app lists injected wallets only, and an empty `VITE_CHAIN_ID` would otherwise fall back to 31337. CSP `img-src` comes from `CSP_IMG_SRC`, which the image defaults because nginx's envsubst leaves undefined variables literal; no build loads Google Fonts. Cloud Run reaches the private-IP Cloud SQL only through Direct VPC egress (`private-ranges-only`). `services replace` applies no IAM, so the script grants `allUsers` `roles/run.invoker` on api, web and web-demo only. GCP facts that the repo can't prove are marked "(inferred; verify before deploy)"; nobody uses WebFetch or WebSearch for them.
- **Settler key rule (D15; 44, T20):** the settler is a dedicated, gas-only EOA, never the deployer, the owner or the Safe. `deploy.py` requires `OPENAD_SETTLER_ADDRESS` off Anvil and pyevm, and refuses the deployer. `set_settler.py` rotates it; on `base` it prints the Safe calldata. Off 31337, the settler process refuses to start when its key owns the vault.
- **Click integrity rule (D15; 45, T13):** campaign serve responses are `private, no-store`, because each carries a one-time token and counts as an impression; lease, house and empty responses stay `public, max-age=<ttl>`, and a CDN may cache `/v1/serve/*/media` only. The burst rule keys on `client_key(request, OPENAD_TRUSTED_PROXY_HOPS)`, in a bounded, HMAC-keyed map; outside dev and test it is skipped when hops are 0, and that is logged once. `api.yaml` sets `OPENAD_SERVE_ENFORCE_ORIGIN=true`; local dev keeps it off.
