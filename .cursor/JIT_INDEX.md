...
- **Real Postgres for Verify:** there's no docker daemon, but Postgres 16 runs locally via `pgserver` in a scratch venv (binaries in `/usr/lib/postgresql/16`). Set `OPENAD_TEST_PG_URL` so `api/tests/test_migrations.py` (and other PG-gated tests) run against it; otherwise they skip. CI does the same with a `postgres:16` service and a "pytest with Postgres" step (PR #15). Parallel worktrees use separate databases on the same pgserver socket: `openad_test` (39) and `openad_test_q` (41), both merged now.
...
- **JIT files location:** since slice B merged, the uncommitted JIT edits ride with the branch checked out in the primary tree (`/home/claude/OpenAd`). Step 37 committed them as `2ce0f5b`. Later edits ride with `chore/launch-final` (draft PR #16, which is step 36), where the planner's CLOSE archive also lands (36b).
