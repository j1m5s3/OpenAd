...
- **Real Postgres for Verify:** there's no docker daemon, but Postgres 16 runs locally via `pgserver` in a scratch venv (binaries in `/usr/lib/postgresql/16`). Set `OPENAD_TEST_PG_URL` so `api/tests/test_migrations.py` (and other PG-gated tests) run against it; otherwise they skip. CI does the same with a `postgres:16` service and a "pytest with Postgres" step (PR #15). Parallel worktrees use separate databases on the same pgserver socket: `openad_test` (39, merged) and `openad_test_q` (41).
...
- **Calendar rule for UI (step 40):** periods are open-ended (`start_k = first_period_start + k·period_seconds`). A period is buyable, time-wise, when `max(0, start_k − lead) ≤ now < end_k` and (`sale_end == 0` or `end_k ≤ sale_end`), with LEASE terms, `lead > 0`, not paused and not leased. See the PROTOCOL §4.1–4.2 `buy` checks and `api/src/openad/services/periods.py`. Never derive a UI state or a period list from the first period alone; list periods from the current index. Tests, e2e and the screenshot script find period rows by their index cell, never by position.
- **Periods endpoint (step 41, accepted 2026-09-25):** on main through `3605473`, `GET /v1/slots/{id}/periods` has no range cap and reads leases one index at a time. 41 caps it at 60 periods (422 `invalid_window`) with one lease query (T19).
